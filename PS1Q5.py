...
class Prob5():
    def __init__(self):
        ...
        ###### START CODE HERE ######
        self.cat = io.imread(r"C:\Users\julia\OneDrive\Documentos\College\Fall 2025\Intro Comp Vision\PS1\PS1-release\cat.bmp").astype(np.float64) / 255.0
        self.dog = io.imread(r"C:\Users\julia\OneDrive\Documentos\College\Fall 2025\Intro Comp Vision\PS1\PS1-release\dog.bmp").astype(np.float64) / 255.0
        ###### END CODE HERE ######
    ...
    def my_conv2d(self, image, filter):
        ...
        for c in range(3):
            conv = convolve(image[:, :, c], filter)
            convolved.append(conv)
        
        convolved_image = np.clip((np.stack(convolved, axis = 2)), 0, 1)
        ###### END CODE HERE ######
    ...
    def prob_5_1(self):
        ...
        axes[0].imshow(identity_image)
        axes[0].set_title("Identity Filter")
        axes[0].axis("off")

        axes[1].imshow(blur_image)
        axes[1].set_title("Blur Filter")
        axes[1].axis("off")

        axes[2].imshow(sobel_image)
        axes[2].set_title("Sobel Filter")
        axes[2].axis("off")

        axes[3].imshow(laplacian_image)
        axes[3].set_title("Laplacian Filter")
        axes[3].axis("off")
    ...
    def create_hybrid_image(self, image1, image2):
        ...
        hybrid_image = np.clip((low_freq + high_freq), 0, 1)
        ###### END CODE HERE ######
        return hybrid_image
    ...
    def prob_5_2(self):
        ...
        plt.title("Hybrid: Dat Cog")
        plt.axis("off")
        plt.show()

        plt.imshow(hybrid_image_vis)
        plt.title("Multiscale Hybrid Image Visualization")
        plt.axis("off")
        plt.show()
